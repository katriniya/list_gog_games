# -*- coding: utf-8 -*-
import json, sys, re, urllib, webbrowser
...
def main(argv):
	gog_json = json.loads(urllib.urlopen(PAGE+PAGE_POSTFIX+'1').read())
...
def parse_games(total_pages, sort_mode):
	...
	for i in range(1, total_pages+1):
		...
		json_page= urllib.urlopen(PAGE+PAGE_POSTFIX+str(i)).read()
...
def html(list_game, file_name):
	...
	for i in range(0,len(list_game)):
		list_item =	'<td style ="text-align:center"><img src="http://'+str(list_game[i][0])+'_100.jpg"/></td>'+'<td><a href="'+PAGE+str(list_game[i][1])+'">'+list_game[i][2].encode("utf-8")+"</a></td>"+"<td style ='text-align:center'>"+str(list_game[i][3])+"%</td>"+"<td style ='text-align:center'>"+str(list_game[i][4])+"</td>"
